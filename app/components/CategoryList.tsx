...
const CategoryList: React.FC<CategoryListProps> = ({
  ...
  // ✅ SMART FILTER - Different field for different types
  let list = [];
  if (type === "deep") {
    list = rawList.filter((item: any) => 
      item && (item.firstTitle || item.name) && 
      (item.firstTitle || item.name || "").trim() !== ""
    );
    console.log(`🔍 Deep categories: ${rawList.length} raw → ${list.length} filtered`);
  } else if (type === "subDeep") {
    list = rawList.filter((item: any) => 
      item && (item.firstTitle || item.name) && 
      (item.firstTitle || item.name || "").trim() !== ""
    );
  ...
  if (!list.length && type !== "subDeep" && type !== "deep") {
    return (
      <div className={`rounded-2xl border-2 border-dashed py-16 text-center transition-all ${
        isDark ? 'bg-[#111C44]/50 border-gray-700' : 'bg-gradient-to-br from-gray-50 to-white border-gray-300'
      }`}>
        <Folder className={`h-16 w-16 mx-auto mb-4 ${isDark ? 'text-gray-600' : 'text-gray-400'}`} />
        <h3 className={`text-xl font-bold mb-2 ${isDark ? 'text-gray-300' : 'text-gray-700'}`}>No categories found</h3>
        <p className={`${isDark ? 'text-gray-500' : 'text-gray-500'}`}>Add your first category to get started</p>
    ...
    return (
      <div className="space-y-3">
        {list.map((item: any) => {
          ...
          const hasSubCategory = item?.hasSubCategory ?? false;
          const imageUrl = item?.imageUri || item?.image;
          const parentName = item?.parentId;
          ...
          return (
            <div
              key={item?._id ?? item?.id}
              onClick={() => onItemClick?.(item)}
              className={`relative rounded-xl p-2 shadow-sm border overflow-hidden cursor-pointer hover:shadow-md transition-all ${
                ...
                <div className="flex-shrink-0 flex flex-col ml-1 w-14">
                  <div className="flex items-center justify-between mb-1 gap-1 px-0.5">
                    ...
                    <div onClick={(e) => e.stopPropagation()}>
                      <QuickToggle
                        checked={imageVisible}
                        onChange={() => onToggleVisibility?.(item, "imageVisible")}
                      />
                    </div>
                  </div>
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-2">
                    ...
                    <div onClick={(e) => e.stopPropagation()}>
                      <QuickToggle
                        checked={item?.isMainCategoryNameVisible ?? true}
                        onChange={() => onToggleVisibility?.(item, "nameVisible")}
                      />
                    </div>
                  </div>
                  ...
                  <div className={`mt-2 rounded-md px-2 py-1.5 flex items-center justify-between ${
                    isDark ? 'bg-[#1B2559]' : 'bg-gray-50'
                  }`}>
                    ...
                    <div onClick={(e) => e.stopPropagation()}>
                      <QuickToggle
                        checked={hasSubCategory}
                        onChange={() => onToggleVisibility?.(item, "hasSubCategory")}
                      />
                    </div>
                  </div>

                  <div className={`mt-1 rounded-md px-2 py-1.5 flex items-center justify-between ${
                    isDark ? 'bg-[#1B2559]' : 'bg-gray-50'
                  }`}>
                    <span className={`text-[9px] font-semibold ${
                      isDark ? 'text-blue-400' : 'text-gray-600'
                    ...
                    <div onClick={(e) => e.stopPropagation()}>
                      <QuickToggle
                        checked={mainVisible}
                        onChange={() => onToggleVisibility?.(item, "visible")}
                      />
                    </div>
                  </div>
                </div>
    ...
    return (
      <div className="space-y-3">
        {list.map((item: any) => {
          ...
          const imageVisible = item?.isSubCategoryImageVisible ?? item?.imageVisible ?? false;
          const imageUrl = item?.imageUri || item?.image;
          const parentName = item.parentName || parentNameOverride;

          return (
            <div
              key={item?.documentId ?? item?.id}
              onClick={() => onItemClick?.(item)}
              className={`relative flex gap-2 rounded-xl p-2 shadow-sm border overflow-hidden cursor-pointer hover:shadow-md transition-all w-[240px] ${
                isDark ? 'bg-[#111C44] border-gray-800' : 'bg-white border-gray-200'
              }`}
            >
              <div className="absolute left-0 top-0 h-full w-1 bg-blue-500 rounded-r-xl" />
              ...
              <div className="flex-1 ml-2 flex flex-col h-full">
                <div>
                  <div className="flex items-start justify-between gap-1 h-8">
                    <div className="flex flex-col min-w-0 justify-center">
                      <div className="flex items-center gap-2">
                        <h2 className={`text-sm font-bold leading-tight whitespace-nowrap overflow-hidden text-ellipsis ${
                          isDark ? 'text-white' : 'text-gray-900'
                        }`}>
                          {item?.name}
                        </h2>
                ...
                <div className={`h-6 rounded-md px-2 flex items-center justify-between mt-1 ${
                  isDark ? 'bg-[#1B2559]' : 'bg-gray-50'
                }`}>
  ...
  /* ===============================
     DEEP CHILD CATEGORY - DESIGN 4 (FIXED)
  =============================== */
  if (type === "deep") {
    console.log("🎯 RENDERING DEEP SECTION");
    console.log("📦 deepChildCategories from context:", deepChildCategories);
    ...
    if (!list || list.length === 0) {
      return (
        <div className="flex flex-col items-center justify-center p-8 text-center border-2 border-dashed border-gray-300 rounded-lg bg-gray-50">
          ...
          <p className="text-gray-500 font-medium mb-2">No Deep Categories Found</p>
          <p className="text-xs text-gray-400 max-w-xs">
            Add a new category using the form above to see it listed here.
          </p>
        </div>
      );
    }
    ...
    return (
      <div className="space-y-4">
        {list.map((item: any, index: number) => {
          ...
          const firstTitle = item?.firstTitle || item?.name || "Untitled";
          const secondTitle = item?.secondTitle || "";
          const description = item?.description || "";
          const webviewUrl = item?.webviewUrl || "";
          const originalPrice = item?.originalPrice || 0;
          const currentPrice = item?.currentPrice || 0;
          const gst = item?.gst || 0;
          const minTime = item?.minTime || 0;
          const maxTime = item?.maxTime || 0;
          ...
          return (
            <div
              ...
              className={`rounded-lg p-3 border transition-all cursor-pointer ${
                isDark ? 'bg-[#111C44] border-gray-800 hover:border-purple-500' : 'bg-white border-gray-200 hover:border-purple-400'
              }`}
            >
              {/* Header Row */}
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <div className={`h-10 w-10 flex items-center justify-center rounded-lg border-2 border-purple-900 font-bold ${
                    isDark ? 'bg-[#1B2559] text-purple-400' : 'bg-purple-50 text-purple-900'
                  }`}>
                    {index + 1}
                  </div>
                  <div>
              ...
              <div className="flex gap-3 mt-3">
                ...
                <button
                  ...
                  className="flex-1 py-2 bg-red-50 text-red-700 text-sm font-semibold rounded-lg hover:bg-red-100 transition"
                >
                  Delete
                </button>
              </div>
              ...
              {expandedItems[id] && (
                <div className="mt-4 pt-3 border-t border-gray-200 dark:border-gray-700 space-y-2">
                  <div className="grid grid-cols-2 gap-2 text-sm">
                    ...
                    <div className="text-gray-900">{description || "N/A"}</div>
                    
                    <div className="font-medium text-gray-600">Webview URL:</div>
                    <div className="text-gray-900 truncate">{webviewUrl || "N/A"}</div>
                    
                    <div className="font-medium text-gray-600">Original Price:</div>
                    <div className="text-gray-900">₹{originalPrice}</div>
                    
                    <div className="font-medium text-gray-600">Current Price:</div>
                    <div className="text-gray-900">₹{currentPrice}</div>
                    
                    <div className="font-medium text-gray-600">GST:</div>
                    <div className="text-gray-900">{gst}%</div>
                    
                    <div className="font-medium text-gray-600">Delivery Time:</div>
                    <div className="text-gray-900">{minTime} - {maxTime} min</div>
                  </div>
                </div>
              )}
            </div>
  ...
  /* ===============================
     SUB DEEP CHILD CATEGORY - DESIGN 5
  =============================== */
  if (type === "subDeep") {
    if (isLoadingSubDeep) {
      return (
    ...
    return (
      <div className="space-y-6">
        {list.map((item: any, index: number) => {
          ...
          const id = item?.documentId || item?._id || item?.id || item?.subDeepKey || `subdeep-${index}`;
          const firstTitle = item?.firstTitle || item?.name || "Sub Deep Child";

          return (
            <div
              key={id}
              className={`rounded-lg p-2 border transition-all ${
                isDark ? 'bg-[#111C44] border-gray-800' : 'bg-gray-100 border-gray-300'
              }`}
            >
              <div className="flex items-center mb-3">
                <div className={`h-10 w-10 flex-shrink-0 flex items-center justify-center rounded border-2 border-blue-900 font-bold ${
                  isDark ? 'bg-[#1B2559] text-blue-400' : 'bg-white text-blue-900'
                }`}>
                  {index + 1}
                </div>

                <div className={`flex-1 ml-2 flex items-center justify-between rounded border-2 border-blue-900 px-3 h-10 overflow-hidden ${
                  isDark ? 'bg-[#1B2559]' : 'bg-white'
                }`}>
                  <span className={`font-bold truncate mr-2 text-sm md:text-base ${
                ...
                <button
                  ...
                  className={`ml-2 h-10 px-3 rounded font-bold text-xs whitespace-nowrap transition-colors border ${
                    isDark ? 'bg-[#1B2559] text-red-400 border-gray-700 hover:bg-[#232D65]' : 'bg-gray-200 text-red-600 border-gray-300 hover:bg-gray-300'
                  }`}
                >
                  {expandedItems[id] ? "View Less" : "View More"}
                </button>
              ...
              <div className="flex gap-4 mb-3">
                <button
                  ...
                  className={`flex-1 border py-2 font-bold rounded-md shadow-sm transition ${
                    isDark ? 'bg-[#1B2559] border-gray-700 text-green-400 hover:bg-[#232D65]' : 'bg-white text-green-600 hover:bg-green-50'
                  }`}
                >
                  Edit Category
                </button>
                <button
                  ...
                  className={`flex-1 border py-2 font-bold rounded-md shadow-sm transition ${
                    isDark ? 'bg-[#1B2559] border-gray-700 text-red-400 hover:bg-[#232D65]' : 'bg-white text-red-600 hover:bg-red-50'
                  }`}
                >
                  Delete Category
                </button>
              ...
              {expandedItems[id] && (
                <div className="space-y-2 mt-4">
                  <div className={`border-2 border-blue-900 rounded px-2 py-2 text-sm italic font-bold ${
                    isDark ? 'bg-[#1B2559] text-white' : 'bg-white text-gray-800'
                  }`}>
                    First Title: {item.firstTitle || "N/A"}
                  </div>
                  <div className={`border-2 border-blue-900 rounded px-2 py-2 text-sm italic font-bold ${
