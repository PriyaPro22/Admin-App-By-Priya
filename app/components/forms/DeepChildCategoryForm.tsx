...
import { Save, Upload, IndianRupee, Clock, AlertCircle, ImageIcon, EyeOff } from "lucide-react";
...
const DeepChildCategoryForm: React.FC<DeepChildCategoryFormProps> = ({
  ...
  // 🔥 DRAG OVER (important - warna drop nahi hoga)
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (formData.photo instanceof File) {
        ...
        deepChildData.image = formData.imageUrl.trim();
      }
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className={`relative inline-block h-6 w-12 cursor-pointer rounded-full transition-colors duration-200 ${formData.isDeepVisible ? "bg-green-500" : "bg-gray-300"
                }`}
            >
              <div
                className={`absolute top-0 h-6 w-6 rounded-full bg-white shadow-md transition-transform duration-200 ${formData.isDeepVisible ? "translate-x-6" : "translate-x-0"
                  }`}
              />
            </button>
          </div>
