...
const ChildCategoryForm = ({ onSuccess, editingCategory }: ChildCategoryFormProps) => {
  ...
  useEffect(() => {
    ...
    if (selectedMainCategory.hasSubCategory) {
      fetchSubCategories();
    } else {
      setSubCategories([]);
    }
    ...
    try {
      ...
      for (const type of types) {
        await addChildCategory({
          name: type,
          mainCategoryId: formData.mainCategoryId,
          subCategoryId: selectedMainCategory?.hasSubCategory ? (subIdToUse || undefined) : undefined,
          visible: true,
        });
      }
      ...
      console.error(err);
      alert("Failed to save child category");
    } finally {
      setIsSaving(false);
    }
  };
    ...
    return (
      <div className="rounded-lg border border-blue-500 bg-gray-200 p-4">
        ...
        <button
          ...
          className="mt-4 bg-blue-900 text-white px-4 py-2 rounded flex gap-2 items-center hover:bg-blue-800"
        >
          <Save size={16} /> {isSaving ? "Updating..." : "Update"}
        </button>
      </div>
    );
  }
            ...
            <div className="bg-white border max-h-40 overflow-y-auto">
              {filteredSubCategories.map(sub => (
                <div
                  ...
                  onClick={() => {
                    setSubSearch(sub.name);
                    setFormData(prev => ({ ...prev, subCategoryId: sub.documentId }));
                    setSubOpen(false);
                  }}
                >
                  {sub.name}
